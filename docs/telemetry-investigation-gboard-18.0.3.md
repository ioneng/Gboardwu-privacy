...
Primary runtime and network validation was completed on 2026-09-18 against the supported Gboard 18.0.3 target.

### Device and patching environment

The validation device was an OPPO CPH2765 running Android 16. The patched app used the coexistence package name `dev.jason.com.google.android.inputmethod.latin`. Morphe Manager 1.31.1 reported Patcher 1.14.0.

Because the coexistence build is resigned, the existing `Add Gboard Signature Bypass` patch was selected during runtime testing. Without it, the patched app hit Gboard's certificate whitelist check during cold startup.

### Functional smoke testing

The following exercised paths continued to work after telemetry blocking:

- keyboard startup and ordinary typing;
- voice typing;
- OCR / Scan Text;
- Tenor category and autocomplete requests;
- fresh Tenor search;
- Tenor media download;
- selecting a GIF and sending it to the input box.

Package-scoped logcat inspection did not show the targeted Clearcut or ClientTelemetry terms during the exercised paths. ColorOS log flow control can drop rows, so this log evidence is supportive rather than exhaustive.

### Tenor network validation

PCAPdroid was used for app-scoped traffic capture.

Normal encrypted captures showed expected functional Tenor traffic to `tenor.googleapis.com` and `media.tenor.com`, with no additional connection appearing when a GIF was tapped. A temporary validation-only build then opted into user-installed CA trust so PCAPdroid could decrypt HTTPS. QUIC was blocked during that test so Tenor fell back to decryptable HTTPS.

In the decisive fresh-search test, PCAPdroid showed:

- `GET /v2/search` returning HTTP 200;
- a subsequent `media.tenor.com` GIF fetch returning HTTP 200;
- the selected GIF successfully sent to the input box;
- no `/v2/registershare` request anywhere in the complete decrypted request list.

This runtime result confirms the intended Tenor behavior for the tested build: functional search, media delivery, and user-visible sharing are preserved while the dedicated `/v2/registershare` tracking request is suppressed.
...
The runtime session did not deliberately force every rare Google Play Services availability/error-notification state, nor can package-scoped logs prove the absence of every telemetry event under every feature state. Those paths remain covered primarily by exact-target static analysis and unit/build validation.

For the primary user-facing paths exercised on the supported target, runtime behavior is consistent with the patch design.
